...
class TrajectoryErrorBoundary extends Component<
  ...
  render() {
    if (this.state.error) {
      return (
        <Alert
          variant="danger"
          title="Failed to render trajectory"
          actionLinks={
            <Button variant="link" onClick={() => { this.setState({ error: null }); this.props.onReset(); }}>
              ← Back to runs
            </Button>
          }
        >
...
export const TrajectoriesPanel: React.FC<Props> = ({ filters }) => {
  ...
  if (selectedRun) {
    return (
      <TrajectoryErrorBoundary onReset={() => setSelectedRun(null)}>
        <TrajectoryDetail
          run={selectedRun}
  ...
  return (
    <>
      ...
      >
        This will permanently delete <strong>all</strong> runs, hops, and edge statistics. This cannot be undone.
      </Modal>
    </>
  );
};
